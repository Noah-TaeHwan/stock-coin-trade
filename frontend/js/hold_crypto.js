/* ── 터미널 다크 테마: Highcharts 전역 기본값 ─────────────────────── */
if (typeof Highcharts !== 'undefined') {
  const c = termColors();
  Highcharts.setOptions({
    colors: TERM_PALETTE,
    chart: { backgroundColor: 'transparent', style: { fontFamily: "'Pretendard', sans-serif" } },
    title: { style: { color: c.fg } },
    legend: { itemStyle: { color: c.fg2 }, itemHoverStyle: { color: c.accent } },
    xAxis: { lineColor: c.borderStrong, gridLineColor: c.border, labels: { style: { color: c.fg2 } } },
    yAxis: { gridLineColor: c.border, labels: { style: { color: c.muted } } },
    tooltip: { backgroundColor: c.surface2, borderColor: c.borderStrong, style: { color: c.fg } },
  });
}
...
async function loadPortfolioAnalysis() {
  ...
  try {
    ...
    content.innerHTML = `
      ...
      <div class="mt-5"><h3 class="text-sm font-black" style="color:var(--fg);">자산 배분</h3><div class="mt-3 space-y-3">${allocation.map(item => `<div><div class="flex items-center justify-between text-sm"><span class="font-bold" style="color:var(--fg);"><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${escapeHtml(item.color)};margin-right:6px;"></span>${escapeHtml(item.name)}</span><span style="color:var(--muted);">${fmt(item.value)}원 · <strong style="color:var(--fg);">${item.weight}%</strong></span></div><div style="height:8px;margin-top:7px;border-radius:999px;background:var(--surface-2);overflow:hidden;"><div style="width:${item.weight}%;height:100%;border-radius:inherit;background:${escapeHtml(item.color)};"></div></div></div>`).join('')}</div></div>
      ...
      <div class="mt-6"><h3 class="text-sm font-black" style="color:var(--fg);">✦ 어드바이저 체크리스트</h3><div class="mt-3 space-y-2">${checks.map(item => {
        ...
        return `<div class="rounded-xl p-3" style="border:1px solid ${style.border};background:${style.bg};"><div class="flex items-center gap-2 text-sm font-black" style="color:${style.color};"><span>${style.icon}</span><span>${escapeHtml(item.title)}</span></div><p class="mt-1 text-sm leading-relaxed" style="color:${style.color};opacity:.9;">${escapeHtml(item.message)}</p></div>`;
      }).join('')}</div></div>
...
function renderStockPortfolioCharts(positions, totalEval) {
  ...
  const sharedTooltip = { valueSuffix: '원' };
  ...
  Highcharts.chart('stockSectorChart', {
    ...
    title: { text: null },
    xAxis: { categories: sectorData.map(([name]) => name), tickLength: 0,
      labels: { style: { fontSize: '11px', fontWeight: '700' } } },
    yAxis: { min: 0, title: { text: null },
      labels: { style: { fontSize: '10px' }, formatter() { return Highcharts.numberFormat(this.value, 0, '.', ','); } } },
    tooltip: { ...sharedTooltip, pointFormat: '<b>{point.y:,.0f}원</b><br/>전체의 {point.percentage:.1f}%' },
    ...
    <tr>
      <td>
        <div class="flex items-center gap-3">
          <img src="https://static.upbit.com/logos/${escapeHtml(h.marketCodeOnlySymbol)}.png" alt="" class="h-9 w-9 rounded-full" style="border:1px solid var(--border);">
          <div>
...
function setColorText(id, val, numericVal) {
  ...
  if (!el) return;
  el.textContent = val;
  el.style.color = priceColor(numericVal);
}
