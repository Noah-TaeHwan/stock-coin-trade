  ...
  try {
    ...
    tbody.innerHTML = rows.length ? rows.map(row => {
      ...
      return `<tr style="border-bottom:1px solid var(--border);">
        ...
        <td style="padding:8px 12px;font-weight:700;color:var(--fg);font-size:14px;">${escapeHtml(row.name)}<br><span style="font-size:11px;color:var(--accent-dark);">${escapeHtml(row.symbol)}</span></td>
...
function renderMarkets() {
  ...
  document.getElementById('marketBody').innerHTML = rows.map(item => `<tr class="market-row ${selected?.symbol === item.symbol ? 'selected' : ''}" data-symbol="${escapeHtml(item.symbol)}"><td><div class="font-bold" style="color:var(--fg);">${escapeHtml(item.name)}</div><div class="mt-1 text-xs" style="color:var(--muted);">${escapeHtml(item.description)}${item.actualMultiplier ? ' · 실제 기준은 주문창에서 확인' : ''}</div></td><td class="text-right font-bold" style="color:var(--fg);">${won(item.price)}</td><td class="text-right font-bold" style="color:${item.changeRate >= 0 ? 'var(--up)' : 'var(--down)'};">${item.changeRate >= 0 ? '+' : ''}${escapeHtml(item.changeRate)}%</td><td class="text-right font-bold" style="color:var(--accent);">${won(item.tradeAmountPerUnit)}</td><td class="text-right text-xs" style="color:var(--muted);">${escapeHtml(item.unit)}<br>${item.marginRate === 100 ? '현금 100%' : '증거금 ' + item.marginRate + '%'}</td></tr>`).join('');
...
function renderPositions(positions) { lastPositions = positions; const tbody = document.getElementById('positionBody'); tbody.innerHTML = positions.length ? positions.map(pos => `<tr><td><div class="font-bold" style="color:var(--fg);">${escapeHtml(pos.name)}</div><div class="text-xs" style="color:var(--muted);">${escapeHtml(pos.category)}</div></td><td class="text-right">${Number(pos.quantity).toLocaleString()}${escapeHtml(pos.unit)}</td><td class="text-right font-bold" style="color:var(--accent);">${won(pos.evalAmount)}</td><td class="text-right font-bold" style="color:${pos.pnl >= 0 ? 'var(--up)' : 'var(--down)'};">${pos.pnl >= 0 ? '+' : ''}${won(pos.pnl)}</td></tr>`).join('') : '<tr><td colspan="4" class="px-4 py-6 text-center text-sm" style="color:var(--muted);">보유한 대체자산이 없습니다.</td></tr>'; }
