(() => {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!loggedIn) {
      ...
      run.disabled = true;
      result.innerHTML = '로그인 후 실행할 수 있습니다. <a href="/member/login.html" style="color:#93C5FD;text-decoration:underline">로그인 페이지로 이동 →</a>';
    }
  });

  confirm.addEventListener('change', () => { run.disabled = !(loggedIn && confirm.checked); });
  ...
  run.addEventListener('click', async () => {
    ...
    try {
      ...
      confirm.disabled = !loggedIn;
      run.disabled = !(loggedIn && confirm.checked);
    }
  });
})();
