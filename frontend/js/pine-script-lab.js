...
const META={indicator:{title:'20일 이동평균선',plain:'노란 선은 최근 20개 가격의 평균입니다. 가격이 평균선 위에 있으면 최근 흐름이 상대적으로 강하고, 아래에 있으면 약하다고 읽는 기초 지표입니다.'},ma:{title:'이동평균선 교차',plain:'짧은 5일선이 긴 20일선을 위로 통과한 곳이 BUY, 아래로 통과한 곳이 SELL입니다. 선이 자주 교차하면 횡보장에서 신호가 많아질 수 있습니다.'},rsi:{title:'RSI 과매도·과매수',plain:'RSI가 낮은 30 구간을 벗어나면 BUY, 높은 70 구간에서 내려오면 SELL로 표시했습니다. RSI 하나만으로 실제 주문하지 않고 추세와 함께 확인합니다.'}};
...
function initChart(){chart=LightweightCharts.createChart($('pineChart'),termChartOptions({autoSize:true,localization:{locale:'ko-KR',priceFormatter:p=>fmt(p)}}));priceSeries=chart.addLineSeries({color:TERM_PALETTE[1],lineWidth:2,title:'가격'});line1=chart.addLineSeries({color:TERM_MA_COLORS[5],lineWidth:2,priceLineVisible:false,lastValueVisible:true});line2=chart.addLineSeries({color:TERM_MA_COLORS[60],lineWidth:2,priceLineVisible:false,lastValueVisible:true});priceSeries.setData(prices.map((value,i)=>({time:times[i],value})))}
