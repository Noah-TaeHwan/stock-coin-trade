...
이 기록은 **브랜딩 변경 전** 화면과 당시 미커밋 로컬 실행 상태를 검증한 역사적 증거다. 당시 스크린샷 3개는 과거 커밋 `ae363e1`에 남아 있으며 현재 브랜치에서는 제거했다. 이 기록을 현재 `Stock & Coin Trading Desk — Noah` 화면의 재검증 결과로 해석하지 않는다. 아래 수치는 당시 관측값 그대로다.

변경 후 화면과 별도 로컬 실행 결과는 [포트폴리오 브랜딩 검증](portfolio-brand-2026-09-23.md)에 기록했다.
...
브랜딩 전 모의 매수·거래 내역·DB 재시작 화면은 과거 커밋 `ae363e1`의 역사적 캡처로 확인할 수 있다. 현재 브랜치의 [브랜딩 후 화면 캡처](portfolio-brand-2026-09-23.md)는 별도 검증 기록에 있다.

Python 표준 라이브러리로 실행한 API 사전 검사와 재시작 후 검사는 각각 종료 0이다. 별도 검증 세션의 검사 코드·JSON 결과를 확인하고 브라우저 계정의 상태도 직접 대조했다. 원시 빌드 로그와 임시 검사 스크립트는 로컬 자료이며 이 저장소에는 포함하지 않는다. 현재 브랜치의 증거는 위 JSON과 실행 안내이며, 당시 화면 캡처는 과거 커밋에 남아 있다. 같은 흐름을 수동 재현하는 절차는 실행 안내에 있다.

## 남은 문제와 범위

거래 내역의 수량(`frontend/js/stock.js:651`)과 보유 포지션 평균단가(`:621`)가 `rgba(255,255,255,0.7)`로 지정돼 밝은 배경에서 읽기 어렵다. 거래 내역 수량은 스크린샷과 DOM 계산 색상으로 재현했으며, 평균단가도 같은 계산 색상을 확인했다. 아직 수정하지 않았고 전체 UI 검증 통과로 해석하지 않는다. (이후 [터미널형 UI 전환](terminal-ui-2026-09-24.md)에서 두 글자색을 토큰 색으로 바꿨다.)
