...
| 스케줄 | `python-stock-backend/scheduler.py` | `price_sources.allowed("dart")`이고 `DART_API_KEY`가 있을 때만 5분 수집 + 매시 30분 하루 전체 훑기 |
| API | `GET /api/disclosures?date=&symbol=&kind=&risk=&limit=` | DB에서 읽기. 처음 본 시각(KST), DART 원문 링크, 출처 문구. `SOURCE_DEPENDENT_BLUEPRINTS`에 `("dart",)` → public에서 404 |
| 화면 | `frontend/events.html`, `frontend/js/events.js` | 날짜·종목·유형·위험 필터, 위험 행 강조(색 + ⚠ 글자), 판정 주체와 모델 판단 확률, 유형별 건수 |
| 명령 | `frontend/js/common.js` | `DISC 005930`(별칭 `DART`) → `/events.html?symbol=005930`. 인자를 받는 명령을 6자리 코드 검사보다 먼저 처리하도록 순서를 고쳤다 |
| 평가 | `evals/jev_disclosures/`, `src/deskjev/eval_disclosures.py` | 실제 제목 285건 + 따로 둔 55건, `--mode oracle|rules|live` |
...
| DART 합계 | 키당 약 20,000건(비공식) 한도의 약 2.6% | 약 520회 |
| Jev | 2026-09-23 상장사 420건 중 규칙이 비운 23건 | 약 23회, $0.0017(월 약 $0.036) |

## 검증 (2026-09-26, `sct-test:dev` = CI와 같은 python 3.11 이미지)

| 명령 | 결과 |
|---|---|
| `ruff check python-stock-backend src tests` | All checks passed |
| `ruff format --check`(바꾼 src 파일 + `tests/unit tests/integration tests/conftest.py`) | 29 files already formatted |
| `python -m pytest -q -m "not integration" tests/unit tests/policy` | 318 passed, 1 skipped(네트워크) |
| `pytest tests/integration/test_bootstrap_mariadb.py`(일회용 `mariadb:11.4`, `db.sql` 적재) | 3 passed |
| `node --check frontend/js/common.js frontend/js/events.js` | 통과 |
| 명령 바 모의(Node VM: `DISC 005930`, `disc 005930`, `DISC`, `ARB ETH`, `005930`, `STK 005930`, `COIN BTC`, `KRW-ETH`, `ARB`) | 9/9 기대 주소 |
| `eval_disclosures --mode oracle` | 285/285, 오류 0 |
| 수집기 시험(실제 DART, 가짜 저장소, Jev 끔) | 오늘(토) 1호출 013 → 0건. 9/23 빈 저장소 7호출 420건(E 0건 저장), 다시 1호출 0건, 전체 훑기 7호출 0건 |

브라우저 확인과 로컬 스택 기동은 PM 몫이라 하지 않았다(미검증).

## 한계

- 제목만 본다. 본문(document.xml)은 읽지 않는다. 제목과 본문이 다를 수 있다.
- 라벨 검수 전이다. 판단이 갈리는 라벨(기술적 거래정지, 회생 개시신청 기각, 회생계획인가, 대출원리금 연체)은 평가 README에 적었다.
- 목록 순서가 바뀌는 방식은 하루치 관찰로만 추정했다. 5분 수집이 놓친 것은 매시 전체 훑기가 채우지만, 23시 30분 이후 자정 전에 올라온 공시를 5분 수집이 놓치면 다음 날에는 그날 목록만 보므로 채우지 못한다.
- 다른 화면은 `common.js?v=20260925-terminal`로 캐시되어 있어, 새 버전을 받기 전까지 그 화면의 명령 바에서 `DISC`가 동작하지 않을 수 있다. 이번 범위는 명령 표·순서만이라 다른 화면의 버전 문자열은 바꾸지 않았다.
