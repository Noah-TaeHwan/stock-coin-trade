...
def trade_environment() -> dict[str, str]:
    ...
    if not paper_key or not paper_secret:
        raise ValueError("KIS_PAPER_APP_KEY and KIS_PAPER_APP_SECRET must both be set in .env or environment")
    if not re.fullmatch(r"\d{8}-\d{2}", account):
        raise ValueError("KIS paper account must have the form 12345678-01")

    # Drop inherited KIS variables; the operator must verify the selected keys are paper keys.
...
def main() -> int:
    ...
    if not executable:
        ...
        return 1
    try:
        env = trade_environment() if kind == "trade" else {name: value for name, value in os.environ.items() if not name.startswith("KIS_")}
